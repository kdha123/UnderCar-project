...
public class MemberDAO {
	...
	public List<MemberDTO> list(PageObject pageObject) throws Exception {
		...
		// 아이디, 이름, 생년월일, 성별, 전화번호, 등급
		// 페이징 처리를 위해서 rnum추가하고 between startRow and EndRow로 가져온다.
		String sql = " select id, name, " + " to_char(birth, 'yyyy-mm-dd') birth, gender, tel, grade "
				+ " from member ";
		sql += SearchCondition.getMemberSearchSQLWithWhere(pageObject);
		...
		//실행이 되었을 때 dto에 아이디, 이름, 생년월일, 성별, 전화번호, 등급을 담고 list에 추가한다.
		if (rs != null) {
			while (rs.next()) {
				if (list == null)
	...
	public String findId(String email)throws Exception{
		String id = null;
		System.out.println("MemberDAO.findId() " + email );
		Connection con = DBUtil.getConnection();
		...
		if(rs!= null && rs.next()) {
			id = rs.getString("id");
			System.out.println("아이디찾기 성공" + id);
		}
		DBUtil.close(con, pstmt, rs);
		return id;
	}//end of findId()
	...
	public String findPw(String id, String email)throws Exception{
		String pw = null;
		System.out.println("MemberDAO.findPw() "+id+", "+email);
		Connection con = DBUtil.getConnection();
