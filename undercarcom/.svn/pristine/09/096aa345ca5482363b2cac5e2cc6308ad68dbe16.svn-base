...
$(".pagination > li:not('.active')").click(
		function() {
			...
			location = "adminlist.do?page=" + page
					+ "&perPageNum=${pageObject.perPageNum}"
					+ "&key=${pageObject.key}"
...
<div class="container">
 ...
 <div id="searchDiv">
			<form action="adminlist.do" class="form-inline">
				...
				<div class="form-group">
					<select class="form-control" id="key" name="key">
						...
						<option value="w" ${(param.key == "w")?"selected='selected'":"" }>아이디</option>
						<option value="tc" ${(param.key=="tc")?"selected='selected'":"" }>제목/내용</option>
						<option value="tw" ${(param.key == "tw")?"selected='selected'":""}>제목/아이디</option>
						<option value="cw" ${(param.key == "cw")?"selected='selected'":""} >내용/아이디</option>
